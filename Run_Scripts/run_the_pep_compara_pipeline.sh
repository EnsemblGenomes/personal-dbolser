...
## Compara+Peptide+Pipeline

name=pep_pipeline

## Ensembl
source /homes/dbolser/EG_Places/Devel/lib/libensembl-compara-test/setup.sh

## Add Compara scripts used below
PATH=$PATH:${ENSEMBL_ROOT_DIR}/ensembl-compara/scripts/pipeline

## BioPerl (Compara needs at least 1.6.9)
EG_APIS=/nfs/production/panda/ensemblgenomes/apis
...
## NCBI
ncbi_server=mysql-pan-prod-ensrw
ncbi_db=ncbi_taxonomy



## Where are we going to read the cores from? (Used to build the
## registry below)
#core_server=mysql-staging-1-ro
core_server=mysql-prod-1
#core_server=mysql-prod-2
#core_server=mysql-prod-3
#core_server=mysql-staging-1

## Note, for plants we also need these cores from Ensembl (see code to
## load them below):
...
## Where do you want the hive database?
## (Also called the compara_db in the docs?)
...
## You may need to update_master_db.pl

##   This script will check that all the species found in the Registry
##   are in the compara database, and with up-to-date meta-information
##   (such as the genebuild, etc).  By default, the script only does
##   the comparison. You need to add --nodry-run to allow it to update
##   the master database to make it match the core databases.
...
## NOTE, The division option is buggy. It's actually safe to run
## witout division, because it won't add genomes it doesn't know
## about.
    --division $division \
    --nodry-run
...
## NOTE: The value of --compara should match '-species' in the
## ${registry} (we use the value of 'multi' here)...
...
## DON'T WORRY ABOUT THE COLLECTION HERE, USE edit_collection.pl BELOW
...
update_genome.pl \
    ...
    --species elaeis_oleifera
    --species elaeis_guineensis
    --species elaeis_guineensis_5
...
## Run the load members.

#Load Members Pipeline (this is a pre-pipeline)

#echo \
time \
init_pipeline.pl \
    Bio::EnsEMBL::Compara::PipeConfig::EBI::EG::LoadMembers_conf \
    $($hive_server --details script) --password $($hive_server pass) \
    -collection $division \
    -curr_core_registry $registry \
    -master_db         $($mast_server --details url)$mast_db \
    -production_db_url $($mast_server --details url)ensembl_production \
    -curr_core_sources_locs '' \
    -prev_core_sources_locs '' \
    -reuse_member_db 0 \
    -hive_force_init 0

memb_db=${USER}_load_members_93
url=$($hive_server --details url)$memb_db
...
## EDIT IT!
emacs $ENSEMBL_ROOT_DIR/ensembl-compara/modules/Bio/EnsEMBL/\
Compara/PipeConfig/EBI/EG/ProteinTrees_conf.pm \
$config
...
emacs -nw \
/nfs/production/panda/ensemblgenomes/development/dbolser/\
species_tree_39.treefam.topology.nw
...
init_pipeline.pl $config \
    ...
    --member_db $($hive_server --details url)$memb_db \
    --master_db $($mast_server --details url)$mast_db \
    --ncbi_db   $($ncbi_server --details url)$ncbi_db \
...
## Environment as above (Ensembl, BioPerl)
